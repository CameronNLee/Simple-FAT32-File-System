...
{
    struct thread_arg *t_arg = arg;
    char *diskname, *filename, *buf, *buf2;
    int fs_fd;
    int stat, read = 0;
    ...
    read = fs_read(fs_fd, buf, stat); read_count += read;
    printf("Read file '%s' (%d/%d bytes)\n", filename, read, stat);
    printf("Content of the file:\n");
    printf("%.*s\n\n", stat, buf);


    read = fs_read(fs_fd, buf, 40); read_count += read;
    printf("Read file '%s' (%d/%d bytes)\n", filename, read, stat);
    printf("Content of the file:\n");
    printf("%.*s\n\n", 40, buf);

    read = fs_read(fs_fd, buf, 10000); read_count += read;
    printf("Read file '%s' (%d/%d bytes)\n", filename, read, stat);
    printf("Content of the file:\n");
    printf("%.*s\n\n", 10000, buf);
    ...
    read = fs_read(fs_fd, buf, 409995); read_count += read;
    printf("Read file '%s' (%d/%d bytes)\n", filename, read, stat);
    printf("Content of the file:\n");
    printf("%.*s\n\n", stat - 15080, buf);

    printf("total amount of bytes read: %d out of %d\n", read_count, stat);
...
void thread_fs_add(void *arg)
{
    ...
    int written_count = 0;
    written = fs_write(fs_fd, buf, 5); written_count += written;
    fs_lseek(fs_fd, 2);
    written = fs_write(fs_fd, buf, 6); written_count += written;
