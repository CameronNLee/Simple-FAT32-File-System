...
with where the datablock locations of the file are. We use this to as a guide
to which blocks we should read in. We have a **block_offset** and
**byte_offset** that calculates our offset in terms of blocks and bytes,
rather than just bytes. Our functions then splits off into two if statements,
if there's an offset, and if there's no offset. 
...
offset, we're reading all that's left of the block, so we call **memcpy()**,
and copy **bounce_buf** into **buf** There are two scenarios here, either the 
...
**buf_offset**. Then we set **byte_offset** to 0, ensuring we won't trigger
this if statement again. We have one condition, that is if our **multi_count**
goes  to 0. We just return the **multi_count**, free **bounce_buf**, and
increment the file's offset by the **multi_count**.
...
less than the block size, that means we read up to the middle of the block.
...
of using **block_offset** to iterate through our array of FAT locations, we
...
***fs_write()*** was very similiar to the read part, a lot of the variable
names were the same, and represented the same things; the main difference was
that we had to update the FAT table's entries each time we create a new file.
To do so, we had two functions, one that was used for when we needed to create
one entry, ***get_and_set_fat()***, the other, ***set_multi_fat()*** was used
when we had to set more than one entry in the FAT table.

The ***set_multi_fat()*** function takes care of the case where the file has
multiple data blocks associated with it, which implies multiple fat entries.
Thus, when writing multiple data blocks, this function is called in order to
populate the FAT table entries. The function accounts for two scenarios: one
where we populate contiguous entries, and one where we populate non-contiguous
entries.
...
the index of what we set. If we didn't find anything, we return 0. The main
important thing to note is that this function is used specifically for when
the file we want to write is only one data block big- no more, no less.

Next, we populated **fat_location** with the indexes of the entries of the FAT, 
the same way as we did in read.

Much like the read function, here we split off into several different scenarios
depending on several factors involving the offset, the filesize, etc. One such
scenario we account for is if the amount of data blocks we are writing is equal
to exactly 1, AND if we are "creating" the file for the first time (represented
by the first data block being equal to FAT_EOC).

Another scenario is writing to more than one data block. In here, our code is
...
variable names and how we increment/decrement them are the same as well.
The only different way was that we wrote instead of read. We first had a
**bounce_buf** that held in a whole block that we read to it with
**block_read**. We'd then modify that bounce_buf by using ***memcpy()***, to
copy our **buf** into it, and offsetting **buf** if necessary. We then finally
used **block_write** to write **bounce_buf** into the appropriate block
in our file.

## Testing

Our tester takes into account reading in a very large data file and checking
for various offset discrepancies and variable amounts of blocks we read at a
time. We also test ***fs_lseek()*** for testing ***fs_write()***.
...
Overall, we feel that this project was a really good way to learn more about
how file systems work, and we felt that we've grown as programmers through this
...
Thankfully, we did not require any use of online sources for help, other than
piazza posts.
